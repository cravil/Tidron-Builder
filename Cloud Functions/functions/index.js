...
function logVersion(){
  ...
  exports.paddleIntegration = functions.https.onRequest((req, res) => {
      //If there is passthrough, save on correct location, used for pricing per project
      var paddleIntegrationPath="/paddlePayments/"+md5(req.body.email)+"/";
      if(req.body.passthrough&&req.body.passthrough.length>5){
        paddleIntegrationPath="/paddlePayments"+req.body.passthrough+"/"+md5(req.body.email)+"/";
      }
      return admin.database().ref(paddleIntegrationPath).update(
        req.body
      ).then(function() {
